...
export const fetchEnvironmentSkillInventory = Effect.fn(
  ...
  return yield* executeAuthenticatedEnvironmentHttpRequest({
    ...
    signer: Option.isSome(signer) ? signer : Option.none(),
    remoteAuthorization,
    method: "GET",
    url: (httpBaseUrl) => makeEnvironmentHttpApiUrlBuilder(httpBaseUrl).skills.inventory(),
    timeoutMs: input.timeoutMs ?? DEFAULT_SKILL_INVENTORY_TIMEOUT_MS,
    request: ({ client, headers }) => client.skills.inventory({ headers }),
  });
});
